...
prefix = '{0}.{1}'.format(environment, hostname)
...
def dispatch_value(topic_name, field, value, ts, q=None):
    str = '{0}.{1}.{2} {3} {4}'.format(prefix, topic_name, field, value, ts)
    if q is not None:
        q.append(str)
    else:
        print str
...
def domain_metrics(stats, domain, prefix, ts, q=None):
    for stat_name in stats.keys():
        if stat_name in stats and (type(stats[stat_name]) == int or type(stats[stat_name]) == float):
            dispatch_value(domain, '{0}.{1}.{2}'.format(tags['type'],prefix,stat_name), stats[stat_name], ts, q)

def topic_metrics(stats, domain, tags, ts, q=None):
    for stat_name in stats.keys():
        if stat_name in stats:
            if 'topic' in tags and 'client-id' in tags:
                dispatch_value(domain, '{0}.{1}.topics.{2}.{3}'.format(tags['type'],tags['client-id'],escape_topic(tags['topic']),stat_name), stats[stat_name], ts, q)
            elif 'client-id' in tags and stat_name.count('.') > 0 and len(stat_name.split('.')) > 1:
                topic_part = escape_topic('.'.join(stat_name.split('.')[:-1]))
                stat_part = stat_name.split('.')[-1]
                dispatch_value(domain, '{0}.{1}.topics.{2}.{3}'.format(tags['type'],tags['client-id'],topic_part,stat_part), stats[stat_name], ts, q)
            elif 'client-id' in tags:
                dispatch_value(domain, '{0}.{1}.{2}'.format(tags['type'],tags['client-id'],stat_name), stats[stat_name], ts, q)
            elif 'topic' in tags:
                dispatch_value(domain, '{0}.{1}.topics.{2}.{3}'.format(tags['type'],escape_topic(tags['topic']),tags['name'],stat_name), stats[stat_name], ts, q)
            else:
                dispatch_value(domain, '{0}.{1}.{2}'.format(tags['type'],tags['name'],stat_name), stats[stat_name], ts, q)
...
for domain_name in domain_names:
    ...
    if 'client-id' in tags and 'type' in tags and tags['type'].endswith(('-node-metrics')):
        ...
        domain_metrics(domains_json['value'][domain_name], domain, '{0}'.format(tags['client-id']), timestamp, q)
        ...
        dispatch_value(domain, '{0}.{1}.kafka-metrics-count'.format(tags['type'],tags['client-id']), domains_json['value'][domain_name]['count'], timestamp, q)
    elif 'type' in tags and tags['type'] in ['Partition','Log']:
        dispatch_value(domain, '{0}.topics.{1}.partition-{2}.{3}'.format(tags['type'],escape_topic(tags['topic']),tags['partition'],tags['name']), domains_json['value'][domain_name]['Value'], timestamp, q)
        ...
        dispatch_value(domain, '{0}.{1}'.format(tags['type'],tags['name']), domains_json['value'][domain_name]['Value'], timestamp, q)
    elif 'type' in tags and tags['type'] == 'DelayedOperationPurgatory':
        dispatch_value(domain, '{0}.{1}.{2}'.format(tags['type'],tags['delayedOperation'],tags['name']), domains_json['value'][domain_name]['Value'], timestamp, q)
        ...
        domain_metrics(domains_json['value'][domain_name], domain, 'server', timestamp, q)
    #else:
    #    print tags
